    ...
    <>
      <ScaffoldSection isFullWidth>
        ...
        <div className="-space-y-px">
          <ResourceList>
            {isLoading ||
              (isSuccess && <AuthProvider authProviders={authProviders} isLoading={isLoading} />)}
            {isError && (
      ...
      <AddAuthProviderModal visible={isAddProviderOpen} setVisible={setAddProviderOpen} />
    </>
  )
}
...
const AuthProvider = ({
  ...
  return authProviders.map((provider) => {
    return <ProviderForm config={provider} provider={provider as unknown as Provider} />
  })
}
