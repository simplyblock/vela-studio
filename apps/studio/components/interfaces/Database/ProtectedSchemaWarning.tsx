  ...
  return (
    <>
      ...
      <DialogSection className="space-y-2 prose">
        <p className="text-sm">
          The following schemas are managed by Velaand are currently protected from write
        ...
        <p className="text-sm !mt-4">
          These schemas are critical to the functionality of your Velaproject and hence we
...
export const ProtectedSchemaWarning = ({
  ...
  return (
    ...
    >
      {reason === 'fdw' ? (
        <p>
          The <code className="text-xs">{schema}</code> schema is used by Velato connect to
          analytics buckets and is read-only through the dashboard.
        </p>
      ) : (
        <>
          <p className="mb-2">
            The <code className="text-xs">{schema}</code> schema is managed by Velaand is
