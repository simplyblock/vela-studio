...
const TableEditorPage: NextPageWithLayout = () => {
  ...
  useEffect(() => {
    ...
    if (lastOpenedTab !== undefined) {
      router.push(`/org/${slug}/project/${projectRef}/branch/${branchRef}/editor/${appSnap.dashboardHistory.editor}`)
    } else if (lastTabId) {
      // Handle redirect to last opened table tab, or last table tab
      const lastTab = tabStore.tabsMap[lastTabId]
      if (lastTab) router.push(`/org/${slug}/project/${projectRef}/branch/${branchRef}/editor/${lastTab.metadata?.tableId}`)
    }
  }, [])
