...
export const IS_VELA_PLATFORM = !process.env.NEXT_PUBLIC_PLATFORM || process.env.NEXT_PUBLIC_PLATFORM === 'vela'

export const API_URL = (() => {
  if (process.env.NODE_ENV === 'test') return 'http://localhost:3000/api'
  //  If running in platform, use API_URL from the env var
  if (IS_PLATFORM) return process.env.NEXT_PUBLIC_API_URL || '/api'
